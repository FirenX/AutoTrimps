...
//genBTC advanced settings - option buttons.
    ...
    createSetting('AutoRoboTrimp', 'AutoRoboTrimp', 'Use RoboTrimps ability starting at this level, and every 5 levels thereafter. (set to 0 to disable. default 60.) If this is a negative number it will not get thrown off by Liquified Zones. 60 and -60 are good choices for mostly everybody.', 'valueNegative', '60', null, 'genBTC');
...
// FirenX' mods
    ...
    createSetting('ExplorerCaptoTributes', 'Cap Explorers to a % of Tributes', 'Only Buy an Explorer when its cost is LESS than X% of cost of a tribute. This setting can work in combination with the other one, or set the other one to -1 and this will take full control. Default: -1 (Disabled). 50% is close to the point where the cap does nothing. You can go as low as you want but recommended is 10% to 1%. (example: Explorer cost of 5001, Tribute cost of 100000, @ 5%, it would NOT buy the trainer.)', 'value', '-1', null, 'FirenX');

// Scryer settings
    createSetting('UseScryerStance', 'Use Scryer Stance', '<b>MASTER BUTTON</b> Stay in Scryer stance in z181 and above (Overrides Autostance). Switch to Scryer at the end of Cache Maps to get double the reward. Falls back to regular Autostance when not in use (so leave that on). Get 2x resources or Dark Essence. <u>All other buttons have no effect if this one is off.</u>', 'boolean', true, null, 'Scryer');
    ...
    createSetting('ScryerSkipEmpowered2', ['Maybe Use S on Empowereds', 'Dont Use S on Empowereds'], 'Overkill overrides this setting, even on Dont Use. Turning this Green doesnt use S stance for empowered cells UNLESS you can overkill them. Red/Maybe just means default (empowereds are treated like normal cells), so something else has to be ON to trigger Scryer to be used.', 'multitoggle', 0, null, 'Scryer');
    ...
    createSetting('ScryerUseWhenPoison', 'Use When Poison', 'Use when we have the Poison Empowerment. Exclusively overrides Corrupted and Empowered Setting.', 'boolean', false, null, 'Scryer');
...
// Dimensional Generator settings:
    ...
    createSetting('AutoGen2Max', 'Max Trimp Gain', 'Get as much trimps as possible until Zone Z. Run Maps while Capacity is filled. Use with caution. This WILL slow down early zones BIG time but might be helpful to reach higher zones.  This setting overrides Early Mode.', 'boolean', null, null, 'Magma');

    document.getElementById('AutoGen2Max').parentNode.insertAdjacentHTML('afterend','<hr>');
